...
const Register: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    if (data) {
      const res = await signUp(data) // Gọi hàm đăng ký với dữ liệu từ form
      if (res?.code === 200) history.push('/login') // Chuyển hướng về trang đăng nhập sau khi đăng ký thành công
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Grid container spacing={2} alignItems='center' justifyContent='center' style={styledContainer}>
